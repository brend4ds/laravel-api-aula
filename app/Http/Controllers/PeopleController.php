...
class PeopleController extends Controller
{
    public function list (Request $request){
        ...
        return People::with('interests')->paginate(15); //colocando 10 por pag
    }
    ...
    public function store(StorePeopleRequest $people){
        ...
        if($newPeople){
            return response()->json([
                'message' => 'Nova pessoa criada com sucesso.', 'people' => $newPeople,
            ]);
        } else {
            return response()->json([
    ...
    public function storeInterest(StoreInterestRequest $interest){
        ...
        if($newInterest){
            return response()->json([
                'message' => 'Novo interesse criado com sucesso.', 'interest' => $newInterest,
            ]);
        } else {
            return response()->json([
                'message' => 'Deu ruim. Te vira aí pra descobrir o que aconteceu.'
